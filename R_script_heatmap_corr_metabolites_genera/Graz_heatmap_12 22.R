...
genus_data_t_merged <- genus_data_t_merged %>%
  select(Sample_ID,
         Enterococcus, Lactobacillus, Staphylococcus)
...
#here, for GRaz, only 5 samples remain
...
xtrans <- bestNormalize(genus_table_top$Enterococcus, loo=TRUE)
hist(xtrans$x.t)
...
ggsave("Graz_heatmap_12_22.svg",
       width = 20,
       height = 25,
       units = c("cm"),
       dpi = 300)

write.csv(df,"Graz_heatmap_12_22.csv")
